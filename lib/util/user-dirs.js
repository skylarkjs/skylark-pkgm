...
const FALLBACK_CONFIG_DIR = path.join(userHome, '.config', 'spkgm');
const FALLBACK_CACHE_DIR = path.join(userHome, '.cache', 'spkgm');
...
function getDataDir() {
  if (process.platform === 'win32') {
    ...
    return path.join(process.env.XDG_DATA_HOME, 'spkgm');
  } else {
    ...
    // path.join(userHome, '.local', 'share', 'spkgm')
...
function getCacheDir() {
  if (process.platform === 'win32') {
    ...
    return path.join(process.env.XDG_CACHE_HOME, 'spkgm');
...
function getConfigDir() {
  if (process.platform === 'win32') {
    ...
    return path.join(process.env.XDG_CONFIG_HOME, 'spkgm');
  } else {
    return FALLBACK_CONFIG_DIR;
  }
}
