...
const PATH_KEYS = new Set(['spkgm-path', 'cache-folder', 'global-folder', 'modules-folder', 'cwd', 'offline-cache-folder']);
...
function getRcConfigForCwd(cwd, args) {
  ...
  if (args.indexOf('--no-default-rc') === -1) {
    Object.assign(config, (_rc || _load_rc()).findRc('spkgm', cwd, (fileText, filePath) => {
...
function getRcConfigForFolder(cwd) {
  const filePath = (0, (_path || _load_path()).resolve)(cwd, '.spkgmrc');
