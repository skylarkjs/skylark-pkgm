...
class PackageRequest {
  constructor(req, resolver) {
    ...
    ///console.log(`pattern:${this.pattern},resolver id:${resolver.rid},flat:${resolver.flat}`);
    //console.dir(resolver.resolutionMap);
  }
  ...
  find({ fresh, frozen }) {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      ...
      for (const depName in info.dependencies) {
        ...
        //lwf
        if (_this5.resolver.dependings.includes(depPattern)) {
          ///console.log("existed,depPattern:" + depPattern);

           continue;
        }
        _this5.resolver.dependings.push(depPattern);
        ///console.log("depPattern:" + depPattern);
