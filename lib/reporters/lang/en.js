...
const messages = {
  ...
  shrinkwrapWarning: 'npm-shrinkwrap.json found. This will not be updated or respected. See https://spkgmpkg.com/en/docs/migrating-from-npm for more information.',
  ...
  lockfileConflict: 'A merge conflict was found in spkgm.lock but it could not be successfully merged, regenerating spkgm.lock from scratch.',
  ...
  cleanCreatedFile: 'Created $0. Please review the contents of this file then run "spkgm autoclean --force" to perform a clean.',
  ...
  linkCollision: "There's already a package called $0 registered. This command has had no effect. If this command was run in another folder with the same name, the other folder is still linked. Please run spkgm unlink in the other folder if you want to register this folder.",
  linkMissing: 'No registered package found called $0.',
  linkRegistered: 'Registered $0.',
  linkRegisteredMessage: 'You can now run `spkgm link $0` in the projects where you want to use this package and it will be used instead.',
  linkUnregistered: 'Unregistered $0.',
  linkUnregisteredMessage: 'You can now run `spkgm unlink $0` in the projects where you no longer want to use this package.',
  ...
  createMissingPackage: 'Package not found - this is probably an internal error, and should be reported at https://github.com/spkgmpkg/spkgm/issues.',
  ...
  workspacesDisabled: 'Your project root defines workspaces but the feature is disabled in your Yarn config. Please check "workspaces-experimental" in your .spkgmrc file.',
  ...
  packageRequiresNodeGyp: 'This package requires node-gyp, which is not currently installed. Yarn will attempt to automatically install it. If this fails, you can run "spkgm global add node-gyp" to manually install it.',
  nodeGypAutoInstallFailed: 'Failed to auto-install node-gyp. Please run "spkgm global add node-gyp" manually. Error: $0',
  ...
  fetchBadIntegrityCache: 'Incorrect integrity when fetching from the cache for $0. Cache has $1 and remote has $2. Run `spkgm cache clean` to fix the problem',
  fetchBadHashCache: 'Incorrect hash when fetching from the cache for $0. Cache has $1 and remote has $2. Run `spkgm cache clean` to fix the problem',
  ...
  plugnplaySuggestV2L2: 'Please give it a try and tell us what you think! - https://next.spkgmpkg.com/getting-started/install',
  ...
  packageContainsYarnAsGlobal: 'Installing Yarn via Yarn will result in you having two separate versions of Yarn installed at the same time, which is not recommended. To update Yarn please follow https://spkgmpkg.com/en/docs/install .',
