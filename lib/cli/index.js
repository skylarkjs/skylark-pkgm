...
let main = exports.main = (() => {
  var _ref = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* ({
    ...
    // set global options
    (_commander || _load_commander()).default.version((_spkgmVersion || _load_spkgmVersion()).version, '-v, --version');
    (_commander || _load_commander()).default.usage('[command] [flags]');
    (_commander || _load_commander()).default.option('--no-default-rc', 'prevent Yarn from automatically detecting spkgmrc and npmrc files');
    (_commander || _load_commander()).default.option('--use-spkgmrc <path>', 'specifies a spkgmrc file that Yarn should use (.spkgmrc only, not .npmrc)', collect, []);
    ...
    (_commander || _load_commander()).default.option('--preferred-cache-folder <path>', 'specify a custom folder to store the spkgm cache if possible');
    (_commander || _load_commander()).default.option('--cache-folder <path>', 'specify a custom folder that must be used to store the spkgm cache');
    (_commander || _load_commander()).default.option('--mutex <type>[:specifier]', 'use a mutex to ensure only one spkgm instance is executing');
    ...
    if (args[0] === '-v') {
      console.log((_spkgmVersion || _load_spkgmVersion()).version.trim());
      process.exitCode = 0;
      return;
    }
    ...
    if (shouldWrapOutput) {
      reporter.header(commandName, { name: 'spkgm', version: (_spkgmVersion || _load_spkgmVersion()).version });
    }
    ...
    function onUnexpectedError(err) {
      ...
      log.push(`Yarn version: ${indent((_spkgmVersion || _load_spkgmVersion()).version)}`);
    ...
    function writeErrorReport(log) {
      const errorReportLoc = config.enableMetaFolder ? (_path || _load_path()).default.join(config.cwd, (_constants || _load_constants()).META_FOLDER, 'spkgm-error.log') : (_path || _load_path()).default.join(config.cwd, 'spkgm-error.log');
...
let start = (() => {
  var _ref5 = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
    ...
    if (spkgmPath && !(0, (_conversion || _load_conversion()).boolifyWithDefault)(process.env.YARN_IGNORE_PATH, false)) {
      ...
      try {
        if (/\.[cm]?js$/.test(spkgmPath)) {
          exitCode = yield (0, (_child || _load_child()).spawnp)(process.execPath, [spkgmPath, ...argv], opts);
        } else {
          exitCode = yield (0, (_child || _load_child()).spawnp)(spkgmPath, argv, opts);
        }
      } catch (firstError) {
        try {
          exitCode = yield (0, (_child || _load_child()).forkp)(spkgmPath, argv, opts);
        } catch (error) {
          throw firstError;
        }
      }
...
function _load_spkgmVersion() {
  return _spkgmVersion = require('../util/spkgm-version.js');
}
