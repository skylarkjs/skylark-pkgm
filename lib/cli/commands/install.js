...
let run = exports.run = (() => {
  var _ref31 = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* (config, reporter, flags, args) {
    ...
    if (args.length) {
      ...
      throw new (_errors || _load_errors()).MessageError(reporter.lang(error, `spkgm ${command} ${exampleArgs.join(' ')}`));
    }
...
class Install {
  ...
  init() {
    ...
    ///console.log("install init begin");
  ...
  flatten(patterns) {
    ...
    ///console.log("install flatten begin");
    ...
    ///console.log("install flatten end");
  ...
  checkUpdate() {
    ...
    if ((_spkgmVersion || _load_spkgmVersion()).version.indexOf('-') >= 0) {
      return;
    }
  ...
  _checkUpdate() {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      ...
      _this13.config.registries.spkgm.saveHomeConfig({
        lastUpdateCheck: Date.now()
      });

      if (semver.gt(latestVersion, (_spkgmVersion || _load_spkgmVersion()).version)) {
        const installationMethod = yield (0, (_spkgmVersion || _load_spkgmVersion()).getInstallationMethod)();
        _this13.maybeOutputUpdate = function () {
          _this13.reporter.warn(_this13.reporter.lang('spkgmOutdated', latestVersion, (_spkgmVersion || _load_spkgmVersion()).version));

          const command = getUpdateCommand(installationMethod);
          if (command) {
            _this13.reporter.info(_this13.reporter.lang('spkgmOutdatedCommand'));
            _this13.reporter.command(command);
          } else {
            const installer = getUpdateInstaller(installationMethod);
            if (installer) {
              _this13.reporter.info(_this13.reporter.lang('spkgmOutdatedInstaller', installer));
            }
          }
        };
      }
