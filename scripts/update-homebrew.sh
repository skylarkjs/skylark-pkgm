...
if [ -z "$YARN_VERSION" ]; then
  echo 'Getting Yarn version from https://spkgmpkg.com/latest-version'
  version=`curl --compressed --fail https://spkgmpkg.com/latest-version`
else
...
git branch -D spkgm-$version || true
...
url=https://spkgmpkg.com/downloads/$version/spkgm-v$version.tar.gz
tempfile=`mktemp -t 'spkgm-release-XXXXXXXX.tar.gz'`
curl --compressed --fail -L -o $tempfile $url
hash=`sha256sum $tempfile | head -c 64`

# Update the formula!
brew bump-formula-pr --strict spkgm --url=$url --sha256=$hash --message="This PR was automatically created via a script. Contact @Daniel15 with any questions." --no-browse
