...
version=`./dist/bin/spkgm --version`
tarball="./artifacts/spkgm-v$version.tar.gz"

# Ensure Yarn tarball was built
if [ ! -f "$tarball" ]; then
  echo 'Could not find Yarn tarball; please re-run "spkgm build-dist".'
  exit 1
fi;
...
if [ "`npm view spkgm@$version name`" = 'spkgm' ]; then
  echo "$version is already available on npm"
  exit 0
fi;

# Determine if this is an RC or a stable release
release_type=`curl --compressed --fail https://release.spkgmpkg.com/release_type/$version`
npm_flags=''
